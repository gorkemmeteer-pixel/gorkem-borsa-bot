...
if st.button('🚀 ANALİZİ BAŞLAT'):
    with st.spinner('Analiz yapılıyor...'):
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.success(f"✅ Hedefe Ulaşan ({len(basarili_listesi)})")
            st.write(", ".join(basarili_listesi))
        with c2:
            st.error(f"⏳ Bekleyen ({len(basarisiz_listesi)})")
            st.write(", ".join(basarisiz_listesi[:20]) + "...")

st.sidebar.info("Sistem, alım sinyalinden sonra %3,5 kâr hedefler.")
