...
st.write(f"Hoş geldin Görkem Mete! | ⏰ Sistem Saati: {tr_saati().strftime('%H:%M:%S')}")
...
if st.button('🚀 100 HİSSEYİ ANLIK TARA'):
    with st.spinner('Veriler Midas hızında işleniyor...'):
        sonuclar = []
        guncel_an = tr_saati().strftime('%H:%M:%S') # Her tarama için o anki saati al
        for h in bist_100_elit:
            try:
                ...
                sonuclar.append({
                    "SAAT": guncel_an, # Sütun buraya eklendi
                    "HİSSE": h.replace(".IS", ""),
                    "FİYAT": round(fiyat, 2),
        ...
        st.success(f"Tarama {guncel_an} itibarıyla tamamlandı!")

st.sidebar.info("Görkem, bu veriler 15 dakikalık periyotlarla analiz edilir.")
